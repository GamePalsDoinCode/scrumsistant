...
from ..local_settings import POSTGRES_URL

# Basic Strategy Cribbed From
# https://github.com/jeancochrane/pytest-flask-sqlalchemy/blob/master/pytest_flask_sqlalchemy/fixtures.py
TEST_DB_CONNECTION_STRING = POSTGRES_URL


@pytest.fixture(scope='session')
def db():
    engine = sa.create_engine(TEST_DB_CONNECTION_STRING)
    yield engine.connect()


@pytest.fixture(scope='function')
def db_engine(engine, transaction):
    '''
...
def transaction(request, db):
    ...
    session_factory = sessionmaker(bind=_db.engine)
...
@pytest.fixture(scope='function')
def engine(pytestconfig, request, transaction):
    _transaction = transaction
    '''
    Mock out differentect access to the semi-global Engine object.
    '''
    ...
    #     patch(mocked_engine, new=engine)
