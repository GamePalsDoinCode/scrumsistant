...
class Server:
	def __init__(self):
		...
		redis_pubsub_instance = self.redis.pubsub(
		...
		redis_pubsub_instance.subscribe(**{
			'websocket-IPC': websocket_handler,
			'flask-IPC': flask_handler,
		})
		self._redis_pubsub_thread = redis_pubsub_instance.run_in_thread(
			sleep_time = 0.5,
		)

	def register(self, websocket):
		next_pk = self.redis.incr('WEBSOCKET_PK')
		self.websocket_info_dict[websocket] = WebsocketInfo(
			pk = next_pk,
			username = 'Uninitalized',
		)
		self.redis.set(
			f'owns-connection-{next_pk}',
			self.SERVER_NAME,
		)

	async def unregister(self, websocket):
		dropped_user_info = self.websocket_info_dict.pop(
			websocket,
			None,
		)
		if dropped_user_info:
			self.redis.delete(
				f'owns-connection-{dropped_user_info.pk}'
			)
			message = {
				'type': 'userLeft',
	...
	async def broadcast(self, message, to = None, publish_to_redis = True):
		...
		await asyncio.wait([
			socket.send(message) for socket in to
		])
